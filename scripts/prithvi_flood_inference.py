...
def require_runtime_dependencies(require_preview: bool):
    ...
    print(
        ...
        "inference_requirements.txt instead of mmcv-full."
    )
    print("\nPython 3.12 setup:")
    print("  conda create -n prithvi-flood python=3.12 -y")
    print("  conda activate prithvi-flood")
    print("  pip install torch torchvision --index-url https://download.pytorch.org/whl/cpu")
    print("  pip install --no-build-isolation -r inference_requirements.txt")
...
def download_model_files(model_repo: str, config_file: str, checkpoint_file: str) -> tuple[str, str]:
...
def main() -> int:
    ...
    if not input_path.exists():
        ...
        print("Run scripts/copernicus_flood_segmentation.py first.", file=sys.stderr)
        return 1
    ...
    elif args.config or args.checkpoint:
        print("--config and --checkpoint must be provided together.", file=sys.stderr)
        return 1
    else:
